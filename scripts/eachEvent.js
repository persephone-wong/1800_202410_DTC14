    ...
    db.collection("events")
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          card.querySelector('.check_in_btn').onclick = () => updateCheckin(docID);
    ...
    function updateCheckin(eventDocID) {
      currentUser.get().then((userDoc)=>{
          ...
          if (checkinsUser.includes(eventDocID)){
              ...
              check_ins: firebase.firestore.FieldValue.arrayRemove(eventDocID)}).then(()=>{
                let buttonID = 'checkin-' + eventDocID;
             document.getElementById(buttonID).innerText = 'Check In';
              ...
              console.log("this check-in does not exist in the database and should be added")
              currentUser.update({
               ...
               currentEvent.update({
              typical_wait_time:firebase.firestore.FieldValue.increment(5),
              check_ins: firebase.firestore.FieldValue.arrayUnion(eventDocID)}).then(()=>{
                let buttonID = 'checkin-' + eventDocID;
